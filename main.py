...
def send_verification_email(email, token):
    """
    Надсилає електронну пошту з посиланням для верифікації.
    """
...
async def register_user(user_data: UserRegistration, background_tasks: BackgroundTasks):
    """
    Реєструє нового користувача та надсилає електронний лист для верифікації.
    """
...
async def create_contact(request: Request, contact_data: ContactCreate, avatar: UploadFile = File(...)):
    """
    Створює контакт для підтвердженого користувача та завантажує аватар.
    """
...
class TestApp(unittest.TestCase):
    ...
    def test_register_user_invalid_email(self):
        response = self.client.post(
            "/register/", json={"email": "invalid_email"})
        self.assertEqual(response.status_code, 400)

    def test_register_user_duplicate_email(self):
        self.client.post("/register/", json={"email": "test@example.com"})
        response = self.client.post(
            "/register/", json={"email": "test@example.com"})
        self.assertEqual(response.status_code, 400)
    ...
    def test_verify_email_success(self):
        self.client.post("/register/", json={"email": "test@example.com"})
        token = users["test@example.com"]["verification_token"]
        response = self.client.post(
            "/verify/", data={"email": "test@example.com", "token": token})
        self.assertEqual(response.status_code, 200)

    def test_verify_email_invalid_token(self):
        self.client.post("/register/", json={"email": "test@example.com"})
        response = self.client.post(
            "/verify/", data={"email": "test@example.com", "token": "invalid_token"})
        self.assertEqual(response.status_code, 400)

    def test_verify_email_invalid_email(self):
        response = self.client.post(
            "/verify/", data={"email": "nonexistent@example.com", "token": "some_token"})
        self.assertEqual(response.status_code, 400)
    ...
    def test_create_contact_success(self):
        ...
        self.client.post(
            "/verify/", data={"email": "test@example.com", "token": token})
        response = self.client.post("/create_contact/", json={"email": "test@example.com"}, files={
                                    "avatar": ("avatar.jpg", open("test.jpg", "rb"), "image/jpeg")})
        self.assertEqual(response.status_code, 200)
